---
title: "Unlocking Insights: Predictive Modeling of Anime Show Scores"
subtitle: |
  | Final Project 
...
A preliminary glance at the distribution in @fig-initial-exploration shows that most of the scores lie somewhere between 6 and 7.5.

```{r, echo = FALSE}
#| label: fig-initial-exploration
#| fig-cap: Initial Score Exploration
load(here("exploration_results/initial_score_exploration.rda"))
initial_score_exploration
```

A closer look at the distribution in @fig-second-exploration, when removing the most common score of 6.51, shows the distribution centered at a score of 6.25 with a slight left skew.

```{r, echo= FALSE}
#| label: fig-second-exploration
#| fig-cap: Further Score Exploration
...
The missingness in @fig-missingness will be addressed in the recipes used for the models, where the missing values will be imputed by the mean values of the variables.
...
The feature engineered recipes were made for both trees and parametric models but the only difference was as mentioned above, with the one-hot encoding for the dummy variables.
The main focus of the feature engineering was creating interaction between predictors.
Part of the EDA process was to find correlations between predictors, specifically the number of episodes, the duration of each episodes, and the ranking.
There was a high correlation with with the ranking and score, which makes sense as a score given to anime should likely correlate to the ranking its given as well.
However, it was not included in the recipe as the predictions would not have included the outcome/target variable anyway.

There was a correlation of -0.2054627 between the duration of each episodes and the ranking.
While this is not a high correlation, studies have shown that shorter length episodes are becoming the norm as more and more people have shorter attention spans.
This was previously more common in children television, which is the primary audience of anime, but is now also becoming more common in adult television as well (Jones Medium).
Therefore the interaction between the two variables were included in the feature engineering part of the recipe. More information on this study is located in the references section at the bottom of this report. 

The correlation between ranking and number of episodes was 0.005810827 which shows almost no correlation.
However, the feature engineered recipe still included a step that correlatted and focused on the interaction between the ranking of the anime and the number of episodes because generally, well-liked television tends to run for longer seasons, and therefore more episodes, as production companies continue to air the show.
...
## Tuning
The tuning process for each model required the creation of a tuning grid. For the sake of computational efficiency, a tuning grid with five levels was chosen. 
...
After performing the tuning process, which involved fitting the different sets of hyperparameters onto the folds and evaluating their performance using cross-validation, below in @fig-plot_en_base is a plot that summarizes the performance of the model across different hyperparameter values.

```{r, echo = FALSE}
#| label: fig-plot_en_base
#| fig-cap: Autoplot Performance
...
Looking at @fig-plot_en_base above, one is able to see that the smallest possible value of RMSE is reached by the parameter ranges tuned on the folds.
...
Below in @tbl-plot_en_base are the values for the elastic net model used on the base line recipe.

```{r, echo = FALSE}
#| label: tbl-plot_en_base
#| tbl-cap: Best Parameters
...
Below in @fig-plot_en is a plot of the performance:
...
The performance of the parameter ranges above in @fig-plot_en was extremely identical to the performance on the previous fitting of the kitchen sink recipe.
Following this, the best values for the parameters are listed below in @tbl-plot_en:
...
After performing the tuning process, which involved fitting the different sets of hyperparameters onto the folds and evaluating their performance using cross-validation, below @fig-plot_knn_base summarizes the performance of the model across different hyperparameter values.

```{r, echo = FALSE}
#| label: fig-plot_knn_base
#| fig-cap: Autoplot Performance
...
From @fig-plot_knn_base above, we are able to see that the lowest RMSE is found between the values of 4 and 6 neighbors so in this situation, the decision to do another round/iteration of tuning was not needed as the earlier ranges included values that gave the lowest RMSE.

Below in @tbl-plot_knn_base are the best tuning parameters for the KNN fitting on the kitchen sink recipe where the result gives the lowest RMSE.

```{r, echo = FALSE}
#| label: tbl-plot_knn_base
#| tbl-cap: Best Parameters
...
Below in @fig-plot_knn is an autoplot of its performance.
...
The decision to do another round of tuning was taken because, the left side of @fig-plot_knn showed a steady decrease of the RMSE value as the number of neighbors decreased.
The tuning range for number of neighbors was then **widened to (1,10)**.
Below @fig-plot_knn_2 is the plot showing its performance.

```{r, echo = FALSE}
#| label: fig-plot_knn_2
#| fig-cap: Autoplot Performance
...
The lowest possible RMSE is better highlighted in @fig-plot_knn_2 within this tuning range and so further rounds of tuning were not done.
Below, @tbl-plot_knn shows the best tuning parameters for the KNN fitting on the feature-engineered recipe where the result gives the lowest RMSE.
...
The parameters that were tuned for the random forest model were the minimal node size (min_n) and the number of predictors to try at each split when constructing a decision tree within the random forest (mtry). The number of trees was also a point of tuning, but was not explored within this report due to computational reasons, and was therefore kept at a constant of 1000 trees, to maximize the efficiency of run time for each fitting of the model.
The iterations of tuning were done first on the model using the kitchen sink recipe, then the optimal ranges were used on the feature-engineered model.
On the first round of tuning, default ranges were fitted into the model:
...
Below, @fig-plot_rf_base summarizes the performance of the model across different hyperparameter values.

```{r, echo = FALSE}
#| label: fig-plot_rf_base
#| fig-cap: Autoplot Performance
...
Above, @fig-plot_rf_base shows a steady, near linear, decline of RMSE as the value of mtry increase, so the decision to expand the range of mtry to see if there would be an even smaller RMSE value was made.
The range for **mtry was widened to (1,15)**, where 15 showed the total number of predictors.
There was little variance in RMSE for change in the min_n, so that range was kept the same.

Below, @fig-plot_rf_base_2 is the performance of the new round of tuning.

```{r, echo = FALSE}
#| label: fig-plot_rf_base_2
#| fig-cap: Autoplot Performance
...
In @fig-plot_rf_base_2 the RMSE steadily declines and then levels out as it reached 15 predictors for mtry, which concluded the tuning process.
Below @tbl-plot_rf_base shows the best tuning values and the corresponding RMSE value on the kitchen sink random forest model.

```{r, echo = FALSE}
#| label: tbl-plot_rf_base
#| tbl-cap: Best Parameters
...
For the feature engineered model, the mtry and min_n ranges, (1,15) and (1,10) respectively, were kept for the first round of tuning.
Below is the performance of these ranges on the random forest model fitted with the feature-engineered recipe on the folded data.
...
In @fig-plot_rf there was little variance in the min_n value and the a local mininum of RMSE was achieved with this range of predictors for mtry (the range could not be expanded further as that was the highest number of predictors).

Below in @ tbl-plot_rf are the best tuning values and the corresponding RMSE value on the feature engineered random forest model.
...
@fig-plot_bt_base is the performance of the first round of tuning, fitting the kitchen sink tree recipe on a boosted tree model.

```{r, echo = FALSE}
#| label: fig-plot_bt_base
#| fig-cap: Autoplot Performance
...
While it is hard to distinguish any relation between RMSE values and each parameter in @fig-plot_bt_base, further tuning was done to each parameter at a time to evaluate how they each affect the performance.
When changing the parameter of the minimal node size, little change was done to the RMSE value, so mtry and learning rate needed to be tuned next.
The next round of tuning **changed the learn rate range to (0.09,0.11).** Below, @fig-plot_bt_base_2 shows the performance of the second round of tuning.

```{r, echo = FALSE}
#| label: fig-plot_bt_base_2
#| fig-cap: Autoplot Performance
...
In @fig-plot_bt_base_2 shifting the range of learn rate affected the RMSE values.
For the capabilities of this report, further iterations of tuning did not take place but seeing as there is no significant relationship between the parameters and the RMSE values able to be seen by @fig-plot_bt_base_2, might encourage further rounds of tuning!
Below, @tbl-plot_bt_base shows the best values for the tuning parameters and the corresponding RMSE values for fitting a kitchen sink recipe to a boosted tree model.

```{r, echo = FALSE}
#| label: tbl-plot_bt_base
#| tbl-cap: Best Parameters
...
The previous ranges used on the kitchen sink model were used as for the first round of tuning on the boosted tree, feature engineered model.
Below, @fig-plot_bt shows the performance on this fitting.
...
Since @fig-plot_bt is identical to the previous ones, with no clear relationship, further tuning was not done in this report (although as mentioned before, there may be an avenue of further exploration).
Below, @tbl-plot_bt are the best tuning parameter values and the corresponding RMSE for a boosted tree model fitted with the folded data supplied by the feature engineered tree recipe.
...
To reiterate, RMSE was the chosen metric to evaluate the performance of the different models.
When fitting both kitchen sink and feature engineered recipe on the different models, the RMSE values were always lower on the feature engineered models.
@tbl-rmse shows the RMSEs for each model, including the null model and the two linear models.
...
Again, in @tbl-rmse we can see that feature-engineered models did better than their corresponding kitchen sink model.
This shows that the extra steps added to the recipe, such as removing more predictors and creating interactions between other predictors, helped the model's performance.
**The winning model was the random forest model, using the feature-engineered recipe.** This is because it had the lowest RMSE value, meaning this model's predictions will be the most accurate.
Below, @tbl-params reiterates the best tuning parameters for this model.
...
This is interesting because the general choice is to use between 50 - 70% of the number of predictors!
This may call for further tuning to see if different variations of both min_n and mtry are better and increase performance!

The random forest models generally performing better than any other model was not entirely surprising as random forests are better capable of capturing non-linear relationships between predictors and the target variable, making them suitable for this dataset that had many non-linear relationships between the outcome of score and other variables that measured episode duration, number of episodes, genres, and ratings!
Also, as mentioned earlier, the number of trees was left constant throughout this whole process of tuning due to computational efficiency reasons, so this may create another avenue of tuning and further exploration as more trees help overcome the limitations of single-tree models that do not necessarily have the best predictive accuracy!

# Final Model Analysis:

Following the selection of the best model, the entire training data set was fitted onto the random forest model with the feature engineered recipe.
...
Overall, this analysis not only provides valuable insights into the determinants of anime scores but also underscores the applicability of data science techniques in deciphering audience preferences within the anime industry. By leveraging advanced modeling approaches and rigorous methodology, this study sets the stage for further exploration and refinement, paving the way for continued advancements in predictive analytics within the realm of anime show scoring prediction.


# References:  
Sajid. “Anime Dataset 2023.” Kaggle, 28 July 2023, www.kaggle.com/datasets/dbdmobile/myanimelist-dataset?select=anime-filtered.csv. 
...
Writer, Jones the. “How Long Should TV Shows Be? The Rise of Shorter TV.” Medium, Medium, 24 Nov. 2019, joneswriter.medium.com/how-long-should-tv-shows-be-the-rise-of-shorter-tv-6f23a83d572d. 
